...
This repository contains more targets than actually needed to replicate the associtated publication.
...
The time is based on all targets that have been created so far in the project.
Estimated time will differ depending on the CPU speed and possible availability of a HPC.
Here, we assume that the analysis is run on a cluster with three compute nodes (= "jobs").

```{r README-5, warning=FALSE}
source("_drake.R")
