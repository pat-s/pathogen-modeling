...
diplodia_task <- readRDS("/data/patrick/mod/pathogen-prediction/01-tasks/diplodia-task-dummy-prediction.rda")
...
models_tuned <- pmap(
  ...
  function(models, param_sets, names, tuning_ctrl) {
    if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/05-tuning/{names}-tuned-diplodia.rda"))) {
      parallelStart(
      ...
      out <- tuneParams(models,
                        task = diplodia_task, resampling = rdesc_inner,
                        par.set = param_sets, control = tuning_ctrl,
                        show.info = TRUE, measure = list(brier)
      )
      parallelStop()
      saveRDS(out, glue("/data/patrick/mod/pathogen-prediction/05-tuning/{names}-tuned-diplodia.rda"))
    } else {
      out <- readRDS(glue("/data/patrick/mod/pathogen-prediction/05-tuning/{names}-tuned-diplodia.rda"))
    }
  }
)
...
models_trained <- map(list("ranger", "xgboost", "svm", "binomial", "kknn"), ~ {
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-models/trained-{.x}-diplodia.rda"))) {

    if (!.x == "binomial") {
      tuned <- readRDS(glue("/data/patrick/mod/pathogen-prediction/05-tuning/{.x}-tuned-diplodia.rda"))
    ...
    model_diplodia <- train(lrn, diplodia_task)
    saveRDS(model_diplodia, glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-models/trained-{.x}-diplodia.rda"))
    model_diplodia
  } else {
    model_diplodia <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-models/trained-{.x}-diplodia.rda"))
  }
})
...
predictions <- map2(models_trained, list("ranger", "xgboost", "svm", "binomial", "kknn"), ~ {
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-diplodia.rda"))) {
    if (.y == "xgboost") {
      pred_data <- pred_data[.x$learner.model$feature_names]
    }
    pred <- predict(.x, newdata = pred_data)
    saveRDS(pred, glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-diplodia.rda"))
    pred
  } else {
    predictions <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-diplodia.rda"))
  }
})
...
walk2(predictions, list("ranger", "xgboost", "svm", "binomial", "kknn"), ~ {
  ...
  writeRaster(raster_xg, glue("/data/patrick/mod/pathogen-prediction/07-prediction/tiffs/diplodia-{.y}.tif"),
              overwrite = TRUE
  )
  ...
  png(glue("/data/patrick/mod/pathogen-prediction/08-imgs/single/prediction_{.y}_diplodia.png"))
  plot(map_xgboost)
  dev.off()
})
