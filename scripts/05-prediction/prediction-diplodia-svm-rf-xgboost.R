...
models_trained <- map(list("ranger", "xgboost", "svm"), ~{
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-{.x}-diplodia.rda"))) {
    ...
    saveRDS(model_diplodia, glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-{.x}-diplodia.rda"))
    model_diplodia
  } else {
    model_diplodia <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-{.x}-diplodia.rda"))
  }
})
...
predictions <- map2(models_trained, list("ranger", "xgboost", "svm"), ~{
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predictions-{.y}-diplodia.rda"))) {
    if (.y == "xgboost") {
      pred_data <- pred_data[.x$learner.model$feature_names]
    }
    pred <- predict(.x, newdata = pred_data)
    saveRDS(pred, glue("/data/patrick/mod/pathogen-prediction/07-prediction/predictions-{.y}-diplodia.rda"))
    pred
  } else {
    predictions <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predictions-{.y}-diplodia.rda"))
  }
})
...
walk2(predictions, list("ranger", "xgboost", "svm"), ~{
  ...
  writeRaster(raster_xg, glue("/data/patrick/mod/pathogen-prediction/07-prediction/diplodia-{.y}.tif"),
              overwrite = TRUE
  )
