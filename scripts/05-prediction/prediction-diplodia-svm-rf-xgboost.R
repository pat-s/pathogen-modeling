...
predictions <- map2(models_trained, list("ranger", "xgboost", "svm"), ~{
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-diplodia.rda"))) {
    ...
    predictions <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/spredictions-{.y}-diplodia.rda"))
  }
})
...
walk2(predictions, list("ranger", "xgboost", "svm"), ~{
  ...
  png(glue("/data/patrick/mod/pathogen-prediction/08-imgs/prediction_{.y}_diplodia.png"))
  plot(map_xgboost)
  dev.off()
})
