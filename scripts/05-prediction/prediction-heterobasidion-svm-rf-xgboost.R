...
models_tuned <- pmap(
  ...
  function(models, param_sets, names, tuning_ctrl) {
    if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/05-tuning/{names}-tuned-heterobasidion.rda"))) {
      parallelStart(
      ...
      out <- tuneParams(models,
                        task = heterobasidion_task, resampling = rdesc_inner,
                        par.set = param_sets, control = tuning_ctrl,
                        show.info = TRUE, measure = list(brier)
      )
      parallelStop()
      saveRDS(out, glue("/data/patrick/mod/pathogen-prediction/05-tuning/{names}-tuned-heterobasidion.rda"))
    } else {
      out <- readRDS(glue("/data/patrick/mod/pathogen-prediction/05-tuning/{names}-tuned-heterobasidion.rda"))
    }
  }
)
...
models_trained <- map(list("ranger", "xgboost", "svm", "binomial", "kknn"), ~ {
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-models/trained-{.x}-heterobasidion.rda"))) {

    if (!.x == "binomial") {
      tuned <- readRDS(glue("/data/patrick/mod/pathogen-prediction/05-tuning/{.x}-tuned-heterobasidion.rda"))
    ...
    model_heterobasidion <- train(lrn, heterobasidion_task)
    saveRDS(model_heterobasidion, glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-models/trained-{.x}-heterobasidion.rda"))
    model_heterobasidion
  } else {
    model_heterobasidion <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/trained-models/trained-{.x}-heterobasidion.rda"))
  }
})
...
predictions <- map2(models_trained, list("ranger", "xgboost", "svm", "binomial", "kknn"), ~{
  if (!file.exists(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-heterobasidion.rda"))) {
    if (.y == "xgboost") {
      pred_data <- pred_data[.x$learner.model$feature_names]
    }
    pred <- predict(.x, newdata = pred_data)
    saveRDS(pred, glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-heterobasidion.rda"))
    pred
  } else {
    predictions <- readRDS(glue("/data/patrick/mod/pathogen-prediction/07-prediction/predicted-values/predictions-{.y}-heterobasidion.rda"))
  }
})
...
walk2(predictions, list("ranger", "xgboost", "svm", "binomial", "kknn"), ~{
  ...
  writeRaster(raster_xg, glue("/data/patrick/mod/pathogen-prediction/07-prediction/tiffs/heterobasidion-{.y}.tif"),
              overwrite = TRUE
  )
  ...
  png(glue("/data/patrick/mod/pathogen-prediction/08-imgs/single/prediction_{.y}_heterobasidion.png"))
  plot(map_xgboost)
  dev.off()
})
